...
export async function createPelatihan(data) {
  ...
  const response = await axiosClient.post(`/api/jenis-usaha/add/${data.layanan_id}`, formData, {
...
export async function updatePelatihan(id, data) {
  ...
  // formData.append("bidangUsahaId", data.layanan_id);

  const response = await axiosClient.put(`/api/jenis-usaha/update/${id}`, formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
  return response.data;
}
