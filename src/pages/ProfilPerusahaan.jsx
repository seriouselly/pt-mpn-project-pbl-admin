      ...
      <div className="container-fluid p-0" style={{ maxWidth: '1000px' }}>
        ...
        <div className="profil-section">
          ...
          <div className="row">
            <div className="col-md-8 mb-3">
              ...
              <input type="text" className="form-control-custom" name="name" value={formData.name || ""} onChange={handleChange} />
            </div>
            <div className="col-md-4 mb-3">
              <label className="form-label-custom">Nama Singkat / Brand</label>
              <input type="text" className="form-control-custom" name="short_name" value={formData.short_name || ""} onChange={handleChange} />
            </div>
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Tagline</label>
            <input type="text" className="form-control-custom" name="tagline" value={formData.tagline || ""} onChange={handleChange} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Tentang Perusahaan</label>
            <textarea className="form-control-custom" rows="4" name="about" value={formData.about || ""} onChange={handleChange} />
          </div>
        </div>
        ...
        <div className="profil-section">
          ...
          <div className="mb-4">
            ...
            <textarea className="form-control-custom" rows="2" name="visi" value={formData.visi || ""} onChange={handleChange} />
          </div>
          <div>
            ...
            <div className="mission-list mb-3">
              {(formData.misi || []).map((item, index) => (
                <div key={index} className="mission-item">
                  ...
                  <button className="btn-delete-mission" onClick={() => handleDeleteItem('misi', index)}><Trash2 size={16} /></button>
                </div>
              ))}
            </div>
            <div className="add-mission-wrapper">
              <input type="text" className="form-control-custom" placeholder="Tambah poin misi baru..." value={newMisi} onChange={(e) => setNewMisi(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddItem('misi', newMisi, setNewMisi)} />
              <button className="btn btn-primary d-flex align-items-center gap-2" onClick={() => handleAddItem('misi', newMisi, setNewMisi)}><Plus size={18} /> Tambah</button>
            </div>
          </div>
        </div>
              ...
              <div className="mission-list mb-3">
                {(formData.legalitas || []).map((item, index) => (
                  <div key={index} className="mission-item">
                    ...
                    <button className="btn-delete-mission" onClick={() => handleDeleteItem('legalitas', index)}><X size={16} /></button>
                  </div>
                ))}
              </div>
              <div className="add-mission-wrapper">
                <input type="text" className="form-control-custom" placeholder="Tambah legalitas..." value={newLegalitas} onChange={(e) => setNewLegalitas(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddItem('legalitas', newLegalitas, setNewLegalitas)} />
                <button className="btn btn-outline-primary" onClick={() => handleAddItem('legalitas', newLegalitas, setNewLegalitas)}><Plus size={18} /></button>
              </div>
            </div>
          </div>
          <div className="col-12 col-xl-6">
            <div className="profil-section h-100">
              ...
              <div className="mission-list mb-3">
                {(formData.keunggulan || []).map((item, index) => (
                  <div key={index} className="mission-item">
                    <span className="mission-text" style={{fontSize:'0.9rem'}}>{item}</span>
                    <button className="btn-delete-mission" onClick={() => handleDeleteItem('keunggulan', index)}><X size={16} /></button>
                  </div>
                ))}
              </div>
              <div className="add-mission-wrapper">
                <input type="text" className="form-control-custom" placeholder="Tambah keunggulan..." value={newKeunggulan} onChange={(e) => setNewKeunggulan(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddItem('keunggulan', newKeunggulan, setNewKeunggulan)} />
                <button className="btn btn-outline-primary" onClick={() => handleAddItem('keunggulan', newKeunggulan, setNewKeunggulan)}><Plus size={18} /></button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="profil-section mt-4">
          ...
          <div className="row mb-4">
            <div className="col-md-6 mb-3">
              ...
              <input type="email" className="form-control-custom" value={formData.kontak?.email || ""} onChange={(e) => handleNestedChange('kontak', 'email', e.target.value)} />
            </div>
            <div className="col-md-6 mb-3">
              <label className="form-label-custom">Nomor Telepon</label>
              <input type="text" className="form-control-custom" value={formData.kontak?.phone || ""} onChange={(e) => handleNestedChange('kontak', 'phone', e.target.value)} />
            </div>
            <div className="col-12 mb-3">
              <label className="form-label-custom">Alamat Lengkap</label>
              <textarea className="form-control-custom" rows="2" value={formData.kontak?.address || ""} onChange={(e) => handleNestedChange('kontak', 'address', e.target.value)} />
            </div>
          </div>
        ...
        <div className="profil-actions pb-5">
          <button className="btn btn-outline-secondary d-flex align-items-center gap-2" onClick={loadData}>
